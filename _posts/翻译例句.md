...
- Thus, for view transitions, the view controller that was active when you initiated the transition remains active when the transition finishes.<br />因此，对于视图过渡效果，当你启动过渡效果时视图控制器是活跃当过渡效果完成时会保留活跃状态。
